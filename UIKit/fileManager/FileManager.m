...
#define kNoteFilePath             @"Notes"      //日记文件夹
...
- (void)creatMyWayFilePath {
    ...
    if (!isHasFilePath) {
        ...
        if (isCreatFilePath) {
            ...
            //日记文件夹
            NSString *noteFilePath=[mywayFilePath stringByAppendingPathComponent:kNoteFilePath];
            BOOL isCreatNote=[fileManager createDirectoryAtPath:noteFilePath withIntermediateDirectories:YES attributes:nil error:nil];
            if (isCreatNote) {
                NSLog(@"日记文件夹创建成功");
            }
...
- (void)creatNotePlist:(NSString *)noteName noteData:(NSDictionary *)diction{
    ...
    //日记名字
    NSString *namePath=[NSString stringWithFormat:@"%@.plist",noteName];
    
    NSString *filep=[documentsPath stringByAppendingPathComponent:kMyWayFilePath];
    
    NSString *file=[filep stringByAppendingPathComponent:kNoteFilePath];
    
    NSString *filePath=[file stringByAppendingPathComponent:namePath];
...
- (NSArray *)getAllNoteItems {
    return  [self getAllItemFilePath:kNoteFilePath];
}
...
- (NSDictionary *)getNoteWithFilePath:(NSString *)filePath {
    //读文件
    NSDictionary* diction = [NSDictionary dictionaryWithContentsOfFile:filePath];
    return diction;
}
